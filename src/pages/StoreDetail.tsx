...
const StoreDetail = () => {
  ...
  const productsByCategory = useMemo(() => {
    ...
    for (const p of products) {
      ...
      else if ((p.category as string) === "grocery") grocery.push(p);
    }
    return {
      ...
      grocery: grocery.sort(sortByOrder),
    };
          ...
          {id === SUPERSTORE_ID ? (
            ...
            <Tabs defaultValue={defaultCategory} className="w-full">
              ...
              <TabsList className={`w-full justify-start sm:justify-center overflow-x-auto flex-nowrap h-auto bg-background/95 border border-border/50 shadow-sm rounded-xl ${isNative ? "mb-2 p-1 gap-0.5" : "mb-6 p-1.5 gap-1"} ${availableCategories.length <= 1 ? "hidden" : ""}`}>
                ...
                {availableCategories.includes("grocery") && (
                  <TabsTrigger value="grocery" className="text-sm font-semibold px-5 py-2.5 gap-1.5 data-[state=active]:bg-emerald-500 data-[state=active]:text-white data-[state=active]:shadow-md rounded-lg transition-all">🛒 Grocery ({productsByCategory.grocery.length})</TabsTrigger>
                )}
              </TabsList>
              </div>
                ...
                <TabsContent key={category} value={category}>
                  {(() => {
                    ...
                    const renderQuickFilters = () => {
                      // Pack/Brand quick filter rows hidden across all categories
                      void brandOptions; void packOptions; void filtersActive;
                      void brandFilter; void packFilter; void setBrandFilter; void setPackFilter;
                      return null;
                    };
