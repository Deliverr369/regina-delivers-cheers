...
CREATE MATERIALIZED VIEW internal.public_catalog_groups AS
WITH visible AS (
...
CREATE UNIQUE INDEX public_catalog_groups_pk ON internal.public_catalog_groups (gkey, category);
CREATE INDEX public_catalog_groups_cat_name_idx ON internal.public_catalog_groups (category, gkey);
CREATE INDEX public_catalog_groups_price_idx ON internal.public_catalog_groups (min_price);

GRANT USAGE ON SCHEMA internal TO anon, authenticated, service_role;
GRANT SELECT ON internal.public_catalog_groups TO anon, authenticated;
GRANT ALL ON internal.public_catalog_groups TO service_role;
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY internal.public_catalog_groups;
END;
$$;
